...
import type { SetupPreviews } from "../concierge/preview.js";
 ...
 */
export { draftPatchSchema as reviseSchema } from "@freebirdai/dash-agent";
import { draftPatchSchema as reviseSchema } from "@freebirdai/dash-agent";
...
const stateOf = (draft: ConciergeDraft | null, deps: ConciergeRouteDeps, dashboardId: string) =>
  conciergeState({
    ...
    board: deps.getDashboard(dashboardId),
    previewStatus: (widget) => deps.previews?.status(widget).status ?? "unchecked",
  });
...
export const conciergeRoutes =
  ...
  async (app: FastifyInstance): Promise<void> => {
    ...
    app.post<{ Params: Params; Body: unknown }>(
      "/api/concierge/:dashboardId/preview",
      async (request, reply) => {
        const parsed = z
          .object({
            widget: z.unknown(),
            receipts: z
              .array(z.object({ as: z.string().max(120), receipt: z.string().max(80) }))
              .max(100),
          })
        ...
        const built = buildAll(draft, deps.context(), {
          taken: new Set(board.widgets.map((widget) => widget.id)),
        });
        const current = built.widgets.find((widget) => widget.id === candidate.value!.id);
        if (!current || !deps.previews.matches(current, candidate.value))
          return reply.status(409).send({ error: "The setup has changed. Reload its preview." });
        return deps.previews.validate(current, parsed.data.receipts, board.params.timeZone);
      },
    );
    ...
    app.post<{ Params: Params; Body: unknown }>(
      ...
      async (request, reply) => {
        ...
        if (!offered.some((option) => option.id === parsed.data.arrangement)) {
          return reply.status(409).send({
            error: `"${parsed.data.arrangement}" is not one of the ways these can be shown`,
          });
        }
